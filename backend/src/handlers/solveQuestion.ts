// backend/src/handlers/solveQuestion.ts

import { addQuestionToJson } from '../storage/questionStore.js';
import { Topic, Difficulty, Source } from '../storage/questionStore.js';
...
export async function solveQuestionHandler(body: any): Promise<Response> {
  if (!apiKey) {
  ...
  try {
    ...
    const topic = classificationMatch ? classificationMatch[1].toLowerCase() as Topic : 'unknown';
    ...
    const questionData = {
      ...
      source: 'submitted' as Source, // Indicate the source as 'submitted'
      topic, // Use the extracted topic classification
      difficulty: 'm' as Difficulty, // Set the difficulty level
